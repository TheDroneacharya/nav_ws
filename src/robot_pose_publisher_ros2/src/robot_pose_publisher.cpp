...
private:
	void timer_callback()
	{
		...
		{
			RCLCPP_WARN(this->get_logger(), "Could not transform %s to %s: %s", base_frame.c_str(), map_frame.c_str(), ex.what());
			return;
		}
		...
		if (is_stamped)
		{
			...
			{
				RCLCPP_ERROR(this->get_logger(), "Stamp publisher is uninitialized!");
			}
		}
		else
		{
			...
			{
				RCLCPP_ERROR(this->get_logger(), "Unstamped publisher is uninitialized!");
			}
		}
	}
